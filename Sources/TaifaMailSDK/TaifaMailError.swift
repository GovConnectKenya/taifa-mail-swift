...
public struct TaifaMailError: Error, LocalizedError, CustomStringConvertible {
    ...
    public var description: String {
        if let code {
            return "TaifaMailError(status: \(status), code: \(code), message: \(message))"
        }
        return "TaifaMailError(status: \(status), message: \(message))"
    }
}
